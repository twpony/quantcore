// RollingCov.h — Rolling covariance (滚动协方差, ddof=0)
// Phase: 远期扩展
//
// Mathematical form:  rolling_cov(X, Y, n)[i] = population covariance of window
// Reference:  pandas.Series.rolling(n).cov(other, ddof=0)  — verified match
//
// Computes the population covariance (ddof=0, divisor=n) of the most recent
// `n` observations of two series X and Y.  For i < n-1, evaluateScalar returns NaN.
//
...
struct RollingCovOp {
    static constexpr RollingOpCode kOpCode = RollingOpCode::ROLLING_COV;
    static constexpr const char* name = "rolling_cov";

    explicit RollingCovOp(std::size_t window) noexcept
        : window_(window) {}
