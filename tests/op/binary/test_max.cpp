// test_max.cpp — unit tests for MaxOp (element-wise maximum)
// Phase: 一期必实现
// Reference: numpy.maximum
//
// Coverage:
//   MaxOp: operator identity, evaluateScalar (normal/NaN/Inf),
...
class MaxOpTest : public ::testing::Test {
protected:
    void SetUp() override {
        lhsSmall_  = {1.0, -2.5, 3.14, -100.0, 0.0};
        rhsSmall_  = {0.5, -1.0, 5.0,   -50.0,  2.0};
        // max(lhs, rhs) element-wise
        expectedSmall_ = {1.0, -1.0, 5.0, -50.0, 2.0};
    }
...
// ============================================================
// MaxOp — Operator identity
// ============================================================

TEST_F(MaxOpTest, OpCodeIsMax) {
    EXPECT_EQ(op_.opCode(), BinaryOpCode::MAX);
}

TEST_F(MaxOpTest, NameIsMax) {
    EXPECT_STREQ(op_.opName(), "max");
}

TEST_F(MaxOpTest, CompileTimeConstants) {
    static_assert(MaxOp::kOpCode == BinaryOpCode::MAX);
    EXPECT_STREQ(MaxOp::name, "max");
}

// ============================================================
// MaxOp — evaluateScalar: normal values
// ============================================================

TEST_F(MaxOpTest, EvaluateScalarBasic) {
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(3.0, 1.0), 3.0);
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(1.0, 5.0), 5.0);
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(2.0, 2.0), 2.0);  // equal
}

TEST_F(MaxOpTest, EvaluateScalarNegative) {
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(-1.0, -5.0), -1.0);
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(-10.0, -3.0), -3.0);
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(-7.0, -7.0), -7.0);
}

TEST_F(MaxOpTest, EvaluateScalarMixedSign) {
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(1.0, -5.0), 1.0);
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(-3.0, 2.0), 2.0);
}

TEST_F(MaxOpTest, EvaluateScalarZero) {
...
// ============================================================
// MaxOp — evaluateScalar: special values (NaN, Inf)
// ============================================================

TEST_F(MaxOpTest, EvaluateScalarNaN) {
    double nan = std::numeric_limits<double>::quiet_NaN();
    // NaN propagates: std::max(NaN, x) = NaN, std::max(x, NaN) = NaN
...
TEST_F(MaxOpTest, EvaluateScalarInf) {
    double inf = std::numeric_limits<double>::infinity();
    // +Inf dominates everything except NaN
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(inf, 1.0), inf);
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(1.0, inf), inf);
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(inf, inf), inf);
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(inf, -inf), inf);
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(-inf, inf), inf);
}

TEST_F(MaxOpTest, EvaluateScalarNegativeInf) {
    double negInf = -std::numeric_limits<double>::infinity();
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(negInf, 1.0), 1.0);
    EXPECT_DOUBLE_EQ(op_.evaluateScalar(negInf, negInf), negInf);
}

// ============================================================
// MaxOp — evaluateScalar: consistency with std::max
// ============================================================

TEST_F(MaxOpTest, EvaluateScalarMatchesStdMax) {
    double testVals[] = {
    ...
    for (double a : testVals) {
        for (double b : testVals) {
            ...
            // Our operator propagates NaN (unlike std::max)
            if (std::isnan(a) || std::isnan(b)) {
                EXPECT_TRUE(std::isnan(result)) << "a=" << a << " b=" << b;
            } else {
                EXPECT_DOUBLE_EQ(result, std::max(a, b)) << "a=" << a << " b=" << b;
            }
        }
    }
}
...
// ============================================================
// MaxOp — Batch evaluate: Column vs Column (no nulls)
// ============================================================

TEST_F(MaxOpTest, BatchEvaluateColumnVsColumn) {
...
TEST_F(MaxOpTest, BatchEvaluateLargeArray) {
    ...
    for (std::size_t i = 0; i < N; ++i) {
        EXPECT_DOUBLE_EQ(output[i], std::max(lhs[i], rhs[i])) << "i=" << i;
    }
}

// ============================================================
// MaxOp — Batch evaluate: Column vs Scalar
// ============================================================

TEST_F(MaxOpTest, BatchEvaluateColumnVsScalar) {
    ...
    // max(lhs[i], 1.0)
    EXPECT_DOUBLE_EQ(output[0], 1.0);   // max(-3, 1)
    EXPECT_DOUBLE_EQ(output[1], 1.0);   // max(0, 1)
    EXPECT_DOUBLE_EQ(output[2], 5.0);   // max(5, 1)
    EXPECT_DOUBLE_EQ(output[3], 1.0);   // max(-1, 1)
    EXPECT_DOUBLE_EQ(output[4], 2.0);   // max(2, 1)
}

// ============================================================
// MaxOp — Batch evaluate: Scalar vs Column
// ============================================================

TEST_F(MaxOpTest, BatchEvaluateScalarVsColumn) {
    constexpr std::size_t N = 5;
    ...
    // max(1.0, rhs[i]) — symmetric, same result
    EXPECT_DOUBLE_EQ(output[0], 1.0);
    EXPECT_DOUBLE_EQ(output[1], 1.0);
    EXPECT_DOUBLE_EQ(output[2], 5.0);
    EXPECT_DOUBLE_EQ(output[3], 1.0);
    EXPECT_DOUBLE_EQ(output[4], 2.0);
}

// ============================================================
// MaxOp — Batch evaluate with null mask
// ============================================================

TEST_F(MaxOpTest, BatchEvaluateWithNulls) {
    ...
    EXPECT_DOUBLE_EQ(output[0], 8.0);  // max(1, 8) non-null
    EXPECT_DOUBLE_EQ(output[1], 0.0);  // null
    EXPECT_DOUBLE_EQ(output[2], 6.0);  // max(3, 6) non-null
    EXPECT_DOUBLE_EQ(output[3], 0.0);  // null
    EXPECT_DOUBLE_EQ(output[4], 5.0);  // max(5, 4) non-null
    EXPECT_DOUBLE_EQ(output[5], 0.0);  // null
    EXPECT_DOUBLE_EQ(output[6], 7.0);  // max(7, 2) non-null
    EXPECT_DOUBLE_EQ(output[7], 8.0);  // max(8, 1) non-null
}

TEST_F(MaxOpTest, BatchEvaluateAllNull) {
...
TEST_F(MaxOpTest, BatchEvaluateScalarRhsWithNulls) {
    constexpr std::size_t N = 4;
    ...
    EXPECT_DOUBLE_EQ(output[0], 0.0);   // max(-5, 0) non-null
    EXPECT_DOUBLE_EQ(output[1], 3.0);   // max(3, 0) non-null
    EXPECT_DOUBLE_EQ(output[2], 0.0);   // null
    EXPECT_DOUBLE_EQ(output[3], 10.0);  // max(10, 0) non-null
}

// ============================================================
// MaxOp — SIMD kernel cross-validation
// ============================================================

TEST_F(MaxOpTest, SimdMatchesScalarReference) {
...
TEST_F(MaxOpTest, SimdSmallArray) {
    ...
    EXPECT_DOUBLE_EQ(output[0], 3.0);   // max(3, 1)
    EXPECT_DOUBLE_EQ(output[1], -2.0);  // max(-4, -2)
    EXPECT_DOUBLE_EQ(output[2], 2.0);   // max(0, 2)
    EXPECT_DOUBLE_EQ(output[3], 5.0);   // max(5, 5)
}

// ============================================================
// MaxOp — CRTP dispatch verification
// ============================================================

TEST_F(MaxOpTest, CrtpDispatchMatchesDirectCall) {
...
// ============================================================
// MaxOp — Stateless (multiple instances produce same results)
// ============================================================

TEST_F(MaxOpTest, MultipleInstancesIdentical) {
    MaxOp op1, op2;
