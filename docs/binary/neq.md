...
- **对标参考**: 1) 当X和Y都是向量时，结果是向量Z，其每个元素$z_i=1\ if (x_i\neq y_i) \ else\  0$；2）当X是向量，Y是标量，则结果是向量Z，其每个元素$z_i=1\ if (x_i\neq Y) \ else\  0$；3）当X是标量，Y是标量，则结果是标量Z，$Z=1\ if (X\neq Y) \ else\  0$
- **注意要点**：虽然有3种输入方式，但请使用统一的$Z=NEQ(X,Y)$接口
