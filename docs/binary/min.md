...
- **对标参考**: 1) 当X和Y都是向量时，结果是向量Z，其每个元素$z_i=min(x_i, y_i)$；2）当X是向量，Y是标量，则结果是向量Z，其每个元素$z_i=min(x_i, Y)$；3）当X是标量，Y是标量，则结果是标量Z，$Z=min(X,Y)$
- **注意要点**：虽然有3种输入方式，但请使用统一的$Z=MIN(X,Y)$接口
